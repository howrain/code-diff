...
public class GitAbstractVersionControl extends AbstractVersionControl {
    ...
    public List<VersionControlDto> getDiffCodeClasses(VersionControlDto versionControlDto) {
        ...
        String gitUserName = versionControlDto.getGitUserName();
        String gitPassWord = versionControlDto.getGitPassWord();
        try {
            String localBaseRepoDir = GitRepoUtil.getLocalDir(versionControlDto.getRepoUrl(), customizeConfig.getGitLocalBaseRepoDir(), versionControlDto.getBaseVersion());
            String localNowRepoDir = GitRepoUtil.getLocalDir(versionControlDto.getRepoUrl(), customizeConfig.getGitLocalBaseRepoDir(), versionControlDto.getNowVersion());
            versionControlDto.setNewLocalBasePath(localNowRepoDir);
            versionControlDto.setOldLocalBasePath(localBaseRepoDir);
            //原有代码git对象
            Git baseGit = GitRepoUtil.cloneRepository(versionControlDto.getRepoUrl(), localBaseRepoDir, versionControlDto.getBaseVersion(), gitUserName, gitPassWord);
            //现有代码git对象
            Git nowGit = GitRepoUtil.cloneRepository(versionControlDto.getRepoUrl(), localNowRepoDir, versionControlDto.getNowVersion(),gitUserName, gitPassWord);
            AbstractTreeIterator baseTree = GitRepoUtil.prepareTreeParser(baseGit.getRepository(), versionControlDto.getBaseVersion());
            AbstractTreeIterator nowTree = GitRepoUtil.prepareTreeParser(nowGit.getRepository(), versionControlDto.getNowVersion());
            //获取两个版本之间的差异代码
            ...
            //设置变更行
            versionControlDto.setDiffClasses(new ArrayList<DiffEntryDto>(diffMap.values()));
