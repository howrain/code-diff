...
@Slf4j
@Component
public class SvnDateAbstractVersionControl extends AbstractVersionControl {
    ...
    public CodeManageTypeEnum getType() {
        return CodeManageTypeEnum.SVN_DATE;
    }
    ...
    public List<VersionControlDto> getDiffCodeClasses(VersionControlDto versionControlDto) {
        ...
        // 检索开始、结束时间
        Date startDate = versionControlDto.getStarDate();
        Date endDate = versionControlDto.getEndDate();
        try {
            ...
            // 计算出开始日期的版本号
            long startVersion = svnRepository.getDatedRevision(startDate);
            long endVersion = svnRepository.getDatedRevision(endDate);
            // 开始的版本号需要-1
            startVersion = startVersion - 1 < 0 ? 0 : startVersion - 1;

            SVNRevision oldVersion = SVNRevision.create(startVersion);
            SVNRevision newVersion = SVNRevision.create(endVersion);
