...
public class SvnRepoUtil {
     ...
     * 单独导出svn某一个文件
     * @param repoUrl svn路径
     * @param codePath 本地代码文件目录
     * @param svnRevision svn版本号
     * @param userName svn账号
     * @param password svn密码
     */
    public static void exportFile(String repoUrl, String codePath, SVNRevision svnRevision, String userName, String password) {
        try {
            ISVNOptions options = SVNWCUtil.createDefaultOptions(true);
            SVNUpdateClient updateClient = SVNClientManager.newInstance((DefaultSVNOptions) options, userName, password).getUpdateClient();
            updateClient.doExport(SVNURL.parseURIEncoded(repoUrl), new File(codePath), svnRevision, svnRevision, null, true, SVNDepth.INFINITY);
        } catch (SVNException e) {
            e.printStackTrace();
        }
    }
    ...
    public static SVNLogClient getSvnLogClient(String userName, String password) {
        ISVNOptions options = SVNWCUtil.createDefaultOptions(true);
        //实例化客户端管理类
        return SVNClientManager.newInstance((DefaultSVNOptions) options, userName, password).getLogClient();
    }

    public static SVNRepository getSvnRepository(String userName, String password, SVNURL svnurl) {
        ISVNOptions options = SVNWCUtil.createDefaultOptions(true);
        //实例化客户端管理类
        SVNRepository repository = null;
        try {
            repository = SVNClientManager.newInstance((DefaultSVNOptions) options, userName, password).createRepository(svnurl, false);
        } catch (SVNException e) {
            e.printStackTrace();
        }
        return repository;
